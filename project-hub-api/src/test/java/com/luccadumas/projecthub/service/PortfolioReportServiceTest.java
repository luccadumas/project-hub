...
class PortfolioReportServiceTest {
    ...
    void shouldGeneratePortfolioReport() {
        when(projectRepository.aggregateProjectsByStatus()).thenReturn(List.of(
                new Object[]{ProjectStatus.ENCERRADO, 1L, new BigDecimal("100000")},
                new Object[]{ProjectStatus.EM_ANDAMENTO, 1L, new BigDecimal("50000")}
        ));
        ...
        assertThat(report.getProjectsCountByStatus().get(ProjectStatus.ENCERRADO)).isEqualTo(1L);
        assertThat(report.getProjectsCountByStatus().get(ProjectStatus.EM_ANDAMENTO)).isEqualTo(1L);
